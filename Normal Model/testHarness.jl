...
function NormalBayesExp(seed::Integer, tau0, n_max, frac_fit=.1, avg_tau=4.)
	srand(seed)
	cs = rand(n_max) * 2./frac_fit   
	taus = 2. * avg_tau * rand(n_max)
	NormalBayesExp(cs, taus, zeros(Float64, n_max), zeros(Float64, n_max), tau0)
end
...
function GammaExp(seed::Integer, alpha, beta, n_max, frac_fit = .1, avg_tau=2)
	...
	taus = 2. * avg_tau * rand(n_max)
	GammaExp(cs, thetas, taus, zeros(Float64, n_max), alpha, beta)
end
...
function test1_a(file_out, numRuns, n_grid, seed, tau0)
	...
	o = NormalBayesExp(seed, tau0, maximum(n_grid))
